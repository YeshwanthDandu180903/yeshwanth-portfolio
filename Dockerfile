...
# Copy the backend code (includes faiss_index inside rag_backend_new/)
COPY --chown=user rag_backend_new ./rag_backend_new
...
# We point to rag_backend_new.main and run on port 7860
CMD ["uvicorn", "rag_backend_new.main:app", "--host", "0.0.0.0", "--port", "7860"]
